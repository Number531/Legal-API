...
export class CourtListenerWebSearchClient extends SearchQualityMixin {
  ...
  async searchOpinionsWeb(args) {
    ...
    // Execute Exa search with highlights or full text option
    const results = await this.executeExaSearch(q, validatedLimit, include_snippet, include_full_text);
    ...
    const mapped = filtered.map(r => this.mapOpinionResult(r, include_snippet, include_full_text));
  ...
  async lookupCitationWeb(args) {
    ...
    const results = await this.executeExaSearch(q, validateLimit(limit, 10), include_snippet, include_full_text);
  ...
  async searchDocketsWeb(args) {
    if (!args || typeof args !== 'object') args = {};
    const { case_name, party_name, docket_number, court, date_filed_after, date_filed_before, limit = 3 } = args;
  ...
  async searchAudioWeb(args) {
    if (!args || typeof args !== 'object') args = {};
    const { query, judge_name, court, date_argued_after, date_argued_before, min_duration, has_transcript, limit = 3 } = args;
    const validated = validateLimit(limit, 20);
  ...
  async searchJudgesWeb(args) {
    if (!args || typeof args !== 'object') args = {};
    const { name, court, appointer, selection_method, political_affiliation, limit = 3 } = args;
    const validated = validateLimit(limit, 20);
  ...
  async listCourtsWeb(args) {
    if (!args || typeof args !== 'object') args = {};
    const { jurisdiction, limit = 3 } = args;
  ...
  async executeExaSearch(query, limit, includeSnippet = true, includeFullText = false) {
    ...
    // Configure contents based on what's requested
    let contents;
    if (includeFullText) {
      // Full text mode
      contents = { text: true };
    } else if (includeSnippet) {
      // Highlights mode for smart snippets
      contents = {
        highlights: {
          query: query,  // Use search query for relevance
          numSentences: 3,  // ~300-500 chars
          highlightsPerUrl: 1
        }
      };
    }
  ...
  mapOpinionResult(result, includeSnippet = true, includeFullText = false) {
    const url = result.url || '';
    ...
    if (includeFullText && result.text) {
      ...
      // Highlights mode - Exa returns array of highlight strings
      snippet = Array.isArray(result.highlights) 
        ? result.highlights.join(' ... ')  // Join multiple highlights
        : String(result.highlights || '');
      // Limit to 500 chars if needed
