 ...
 *
 * Migrated to schema-based architecture (Phase 3.6b)
 * Uses PTABSchemas for structured data extraction via Exa summary parameter
 *
 * Searches for:
...
import { PTABSchemas } from './schemas/PTABSchemas.js';
...
export class PTABWebSearchClient extends BaseWebSearchClient {
  constructor(rateLimiter, exaApiKey = process.env.EXA_API_KEY) {
    ...
    // Register PTAB schemas for schema-based extraction
    if (this.contentStrategy) {
      Object.entries(PTABSchemas).forEach(([dataType, schema]) => {
