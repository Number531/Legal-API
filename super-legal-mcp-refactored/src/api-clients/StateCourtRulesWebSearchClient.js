/**
 * State Court Rules Web Search Client (BaseWebSearchClient Integration)
...
import { StateCourtRulesSchemas } from './schemas/StateCourtRulesSchemas.js';

export class StateCourtRulesWebSearchClient extends BaseWebSearchClient {
  constructor(rateLimiter = null, exaApiKey = null) {
    ...
    this.stateCourtDomains = {
      ...
      'CA': { 
        name: 'California', 
        domains: ['courts.ca.gov', 'smartrules.com', 'ceb.com', 'lacourt.org', 'occourts.org', 'sfsuperiorcourt.org', 'sdcourt.ca.gov'],
      ...
      'CT': { 
        name: 'Connecticut', 
        domains: ['jud.ct.gov', 'ctbar.org', 'eservices.jud.ct.gov'],
      ...
      'FL': { 
        name: 'Florida', 
        domains: ['flcourts.org', 'floridabar.org', 'myflcourtaccess.com'],
      ...
      'IL': { 
        name: 'Illinois', 
        domains: ['illinoiscourts.gov', 'isba.org', 'efile.illinoiscourts.gov'],
      ...
      'MD': { 
        name: 'Maryland', 
        domains: ['mdcourts.gov', 'msba.org', 'courts.state.md.us'],
      ...
      'NJ': { 
        name: 'New Jersey', 
        domains: ['njcourts.gov', 'njsba.com', 'portal.njcourts.gov'],
      ...
      'NY': { 
        name: 'New York', 
        domains: ['nycourts.gov', 'courts.state.ny.us', 'nysba.org', 'iapps.courts.state.ny.us'],
      ...
      'PA': { 
        name: 'Pennsylvania', 
        domains: ['pacourts.us', 'pabar.org', 'ujsportal.pacourts.us'],
      ...
      'TX': { 
        name: 'Texas', 
        domains: ['txcourts.gov', 'texasbar.com', 'efile.txcourts.gov'],
      ...
      'WA': { 
        name: 'Washington', 
        domains: ['courts.wa.gov', 'wsba.org', 'efile.courts.wa.gov', 'ac.courts.wa.gov'],
        courtTypes: ['supreme', 'appeals', 'superior', 'district']
      },
      'WV': { 
        name: 'West Virginia', 
      ...
      'WI': { 
        name: 'Wisconsin', 
        domains: ['wicourts.gov', 'wisbar.org', 'efiling.wicourts.gov'],
        courtTypes: ['supreme', 'appeals', 'circuit', 'municipal']
      },
      'WY': { 
        name: 'Wyoming', 
    ...
    // Register State Court Rules schemas for structured data extraction
    if (this.contentStrategy) {
      Object.entries(StateCourtRulesSchemas).forEach(([dataType, schema]) => {
  ...
  async searchCourtRules(args) {
    ...
    try {
      // Enhanced search with BaseWebSearchClient features
      const results = await super.executeExaSearch(searchQuery, validatedLimit, {
        dataType: 'state_court_rule',
        domain: 'state_law',
        summaryQuery: 'court rules procedural requirements formatting guidelines',
      ...
      const fallbackResults = await super.executeExaSearch(fallbackQuery, validatedLimit, {
        dataType: 'state_court_rule',
        domain: 'state_law',
        summaryQuery: 'court rules procedural requirements formatting guidelines',
        includeFullText: include_text
  ...
  async getFormattingRequirements(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'formatting_requirements',
      domain: 'state_law',
      summaryQuery: 'formatting requirements font margins spacing page layout',
      includeDomains: stateInfo.domains,
  ...
  async getElectronicFilingRules(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'electronic_filing_rules',
      domain: 'state_law',
      summaryQuery: 'electronic filing e-filing PDF requirements file size technical specifications',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 6
    });
  ...
  async searchLocalRules(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'local_rules',
      domain: 'state_law',
      summaryQuery: 'local rules county district court specific procedures',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 6
    });
  ...
  async getCourtSpecificProcedures(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'court_procedure',
      domain: 'state_law',
      summaryQuery: 'court procedures motion practice deadlines requirements',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 6
    });
  ...
  async checkRuleUpdates(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'rule_update',
      domain: 'state_law',
      summaryQuery: 'rule changes updates amendments effective date new rules',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 6
    });
  ...
  async getDocumentTemplates(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'document_template',
      domain: 'state_law',
      summaryQuery: 'document template form fillable PDF official sample',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 5
    });
  ...
  async validateDocumentCompliance(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'state_court_rule',
      domain: 'state_law',
      summaryQuery: 'document requirements compliance checklist validation',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 6
    });
  ...
  async getCitationRequirements(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'formatting_requirements',
      domain: 'state_law',
      summaryQuery: 'citation requirements format bluebook style guide references',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 5
    });
  ...
  async getDiscoveryRules(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'court_procedure',
      domain: 'state_law',
      summaryQuery: 'discovery rules interrogatories depositions document production',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 6
    });
  ...
  async getAppellateRequirements(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'court_procedure',
      domain: 'state_law',
      summaryQuery: 'appellate rules brief requirements record preparation deadlines',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 6
    });
  ...
  async getEmergencyProcedures(args) {
    ...
    const results = await super.executeExaSearch(searchQuery, validatedLimit, {
      dataType: 'court_procedure',
      domain: 'state_law',
      summaryQuery: 'emergency procedures TRO preliminary injunction ex parte requirements',
      includeDomains: stateInfo.domains,
      includeFullText: true,
      numSentences: 6
    });
  ...
  // ===== ENHANCED SEARCH METHODS USING BASEWEBSEARCHCLIENT =====
  ...
  mapCourtRuleResult(result, stateInfo, ruleType, includeText) {
    return {
      ...
      ...(result._highlight_quality ? { quality_assessment: result._highlight_quality } : {})
    };
  }
