...
import { FederalRegisterSchemas } from './schemas/FederalRegisterSchemas.js';
...
export class FederalRegisterWebSearchClient extends BaseWebSearchClient {
  constructor(rateLimiter, exaApiKey = process.env.EXA_API_KEY) {
    ...
    // Register Federal Register schemas for structured data extraction
    if (this.contentStrategy) {
      Object.entries(FederalRegisterSchemas).forEach(([dataType, schema]) => {
    ...
    if (this.USE_ENHANCED_QUERIES) {
      ...
      console.log('[FederalRegister] ✨ Enhanced summary queries ENABLED - using context-aware natural language prompts');
    } else {
      this.summaryQueryBuilder = null;
      console.log('[FederalRegister] Enhanced summary queries DISABLED - using static keyword queries (default)');
    }
  }
  ...
  async searchFederalRegisterWeb(args) {
    ...
    if (this.USE_ENHANCED_QUERIES && this.summaryQueryBuilder) {
      try {
        // Extract user's search context (prioritize search_term, then agency)
        ...
        console.warn('[FederalRegister] Enhanced query build failed for searchFederalRegisterWeb, using fallback:', error.message);
      }
    }
    ...
    const results = await this.executeExaSearch(query, validatedLimit, {
      dataType: 'federal_register_document',
  ...
  async searchPublicInspectionWeb(args) {
    ...
    // Build summary query (enhanced or static based on feature flag)
    const baseTerms = 'Federal Register Public Inspection agency rule notice ACTION AGENCY SUMMARY pre-publication';
    let summaryQuery = baseTerms;

    if (this.USE_ENHANCED_QUERIES && this.summaryQueryBuilder) {
      try {
        const userTerm = search_term || agency || 'public inspection';
        summaryQuery = this.summaryQueryBuilder.build({
          userSearchTerm: userTerm,
          dataType: 'federal_register_public_inspection',
          schema: FederalRegisterSchemas.federal_register_document || null,
          baseTerms: baseTerms
        });
      } catch (error) {
        console.warn('[FederalRegister] Enhanced query build failed for searchPublicInspectionWeb, using fallback:', error.message);
      }
    }
