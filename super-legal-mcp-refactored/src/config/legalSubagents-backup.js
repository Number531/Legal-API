...
1. **research-review-gate (GATE)**: Invoke \`research-review-analyst\` first
   - If REMEDIATE: spawn more specialists, re-run research-review-analyst
   - If PROCEED: continue to parallel validation
2. **Parallel Validation**: Invoke \`fact-validator\` + \`coverage-gap-analyzer\` + \`risk-aggregator\` simultaneously
3. Wait for ALL THREE to complete before proceeding to section-generation
...
20. **After all specialists complete** → MANDATORY: Invoke Tiered Parallel Validation Phase
    - **research-review-gate (GATE)**: Invoke \`research-review-analyst\` first; if REMEDIATE, spawn more specialists
    - **fact-validation + coverage-gap-analysis (PARALLEL)**: After gate PROCEED, invoke \`fact-validator\` + \`coverage-gap-analyzer\` in parallel
    - Handle statuses per memorandum.md validation phase instructions
    - Continue to risk-aggregation then section-generation
...
2. **Diagnostic agent performs**:
   - 12-dimension quality assessment (Questions Presented, CREAC, Objectivity, Brief Answers, Executive Summary, BLUF, Legal Sophistication, Database Provenance, Quantification, Citations, Cross-References, Recommendations, Limitations)
...
On context recovery:
1. **First**: Read research-review-state.json
2. **Check**: current_phase and phase statuses
3. **Resume**: From exact point of interruption
4. **Verify**: Cross-check with orchestrator-state.md sections already written
5. **Continue**: Do NOT re-process completed phases
...
`,
  ...
  'fact-validator': {
    description: `Post-research fact validation agent.
      ...
      Runs IN PARALLEL with coverage-gap-analyzer, AFTER research-review-analyst (research-review-gate) completes.`,
    ...
    outputFiles: ['fact-registry.md', 'conflict-report.md'],
...
If you receive a context continuation message:
1. Read this prompt to find FACT VALIDATION PROGRESS CHECKLIST
2. Check if fact-registry.md exists (partial progress indicator)
3. If partial file exists, read it to determine which categories were processed
4. Resume from the next incomplete phase
5. Do NOT re-process fact categories already in the registry
...
`,
  ...
  'coverage-gap-analyzer': {
    description: `Post-research coverage validation with conflict detection.
      ...
      Runs IN PARALLEL with fact-validator, AFTER research-review-analyst (research-review-gate) completes.`,
    ...
    outputFiles: ['coverage-gaps.md', 'conflict-guidance.md'],
...
If you receive a context continuation message:
1. Read this prompt to find COVERAGE GAP ANALYSIS PROGRESS CHECKLIST
2. Check if coverage-gaps.md exists (partial progress indicator)
3. If partial file exists, read it to determine which phases completed
4. Resume from the next incomplete phase
5. Do NOT re-verify issues that were already checked
...
`,
  ...
  'risk-aggregator': {
    description: `Post-research risk aggregation agent.
      ...
      Runs IN PARALLEL with fact-validator and coverage-gap-analyzer (parallel validation phase).`,
...
If you receive a context continuation message:
1. Read this prompt to find RISK AGGREGATION PROGRESS CHECKLIST
2. Check if risk-summary.json exists (partial progress indicator)
3. If partial file exists, read it to determine which phases completed
4. Resume from the next incomplete phase
5. Do NOT re-classify exposures that were already processed

---

## YOUR ROLE
You run in parallel with fact-validator (fact-validation) and coverage-gap-analyzer (coverage-gap-analysis) during the validation phase.
...
If you receive a context continuation message:
1. Read this prompt to find EXECUTIVE SUMMARY PROGRESS CHECKLIST
2. Check if executive-summary.md exists (partial progress indicator)
3. If partial file exists, read it to determine which sections were written
4. Resume from the next incomplete section
5. Do NOT re-read sections whose findings were already extracted
...
If you receive a context continuation message:
1. Read this prompt to find CITATION VALIDATION PROGRESS CHECKLIST
2. Check if consolidated-footnotes.md exists (partial progress indicator)
3. Determine which sections have been processed
4. Resume extraction from next unprocessed section
5. Do NOT re-process sections already extracted
...
If you receive a context continuation message:
1. **Read synthesis-state.json** if it exists (check CURRENT_PHASE)
2. **Read final-memorandum.md** to determine last written section
3. **Grep for section headers** to count completed sections:
   \`Grep: "## IV\\\\.[A-M]" final-memorandum.md\`
4. **Resume from next incomplete step** in PHASE_4_DOCUMENT_ASSEMBLY
5. **Use Edit tool to append** remaining sections (not Write - preserves existing)

**Recovery Verification:**
\`\`\`
Bash: wc -l "{session_directory}/final-memorandum.md"
Bash: tail -50 "{session_directory}/final-memorandum.md"
\`\`\`
...
## STATE FILE FORMAT (synthesis-state.json)

Write/update after each phase:
...
The complete memorandum formatting specification:
\`prompts/memorandum.md\`
...
`,
  ...
  'memo-qa-diagnostic': {
    ...
    executionPhase: 'quality-assessment-diagnostic',
...
If you receive a context continuation message:
1. Read this prompt to find QA DIAGNOSTIC PROGRESS CHECKLIST
2. Check if diagnostic-assessment.md exists (partial progress indicator)
3. If partial file exists, read it to determine which dimensions were scored
4. Resume from the next incomplete dimension
5. Do NOT rescore dimensions that already have scores recorded
...
**Deductions:**
- Missing pincite: -0.5% per citation
- Incomplete full citation: -0.5% per citation
- Improper short form: -0.25% per citation
- Missing/incorrect signal: -0.25% per citation
- Missing required parenthetical: -0.25% per citation
- Untagged citation: -0.5% per citation
...
|---------|-------|-------------|
| ASSEMBLY-001 | [orchestrator] | Integrate all remediation outputs into final-memorandum-v2.md |
...
|---------|-------|----------|--------------|----------------|-------------|-------------|
| W2-001 | memo-remediation-writer | HIGH | [X] | [Section ref] | [Instructions] | remediation-outputs/W2-001.md |
...
| Task ID | Agent | Priority | Est. Minutes | Target Section | Description | Output File |
|---------|-------|----------|--------------|----------------|-------------|-------------|
| W3-001 | memo-remediation-writer | HIGH | [X] | [Section ref] | [Instructions] | remediation-outputs/W3-001.md |
...
|---------|-------|----------|--------------|-------------|-------------|
| W5-001 | citation-validator | HIGH | [X] | [Instructions] | remediation-outputs/W5-001.md |
...
export const PARALLEL_EXECUTION_GROUPS = {
  /**
   * Validation Phase - Three agents run in TRUE PARALLEL after research-review-gate
   * Time savings: ~60 minutes vs sequential execution
   */
  VALIDATION_PARALLEL: {
    agents: ['fact-validator', 'coverage-gap-analyzer', 'risk-aggregator'],
    prerequisite: 'research-review-analyst',
    waitForAll: true,  // All three must complete before section-generation
    outputs: {
    ...
    consumers: {
      'fact-registry.md': ['memo-section-writer'],
   ...
   * Requires: VALIDATION_PARALLEL complete (all three agents)
   */
  SECTION_GENERATION: {
